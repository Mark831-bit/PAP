  ...
  async function refreshStats() {
    try {
      ...
      if (data.scans.length === 0) {
        list.innerHTML = '<p class="empty-state">There are no scans registered yet.</p>';
        return;
      }
      list.innerHTML = '<div class="scans-list">' + data.scans.map(s => {
        ...
        return `<div class="scan-row">
          ...
          <div class="scan-type ${presente ? 'present-text' : 'absent-text'}">${presente ? 'Entry' : 'Exit'}</div>
        </div>`;
      }).join('') + '</div>';
    } catch {}
  }
