...
$uploadDir = __DIR__ . '/../uploads/docs/' . preg_replace('/[^a-zA-Z0-9@._-]/', '_', $login) . '/';
...
if ($action === 'list') {
    ...
    foreach (glob($uploadDir . '*') as $path) {
        ...
        $files[] = [
            ...
            'url'      => '/PAP/uploads/docs/' . rawurlencode(preg_replace('/[^a-zA-Z0-9@._-]/', '_', $login)) . '/' . rawurlencode($name),
        ];
    }
...
if ($action === 'upload' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Safe filename: timestamp + original name sanitised
    ...
    log_event('INFO', 'documento uploaded', ['login' => $login, 'file' => $filename]);
...
if ($action === 'delete' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    unlink($path);
    log_event('INFO', 'documento deleted', ['login' => $login, 'file' => $filename]);
